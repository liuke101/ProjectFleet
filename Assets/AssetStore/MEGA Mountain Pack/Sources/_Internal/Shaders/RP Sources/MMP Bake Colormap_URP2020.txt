...
Shader "Hidden/MMP/Bake Colormap"
{
   Properties
   {
	...
	[NoScaleOffset] _BaseMap("Normal Map", 2D) = "white" {}
    _OcclusionIntensity("Occlusion Intensity", Range(0, 1)) = 0.5
    _OcclusionToAlbedo("Occlusion To Albedo", Range(0, 1)) = 0.5

    [Header(Rock Settings)]
	[NoScaleOffset] _Rock("Rock Layer Texture", 2D) = "white" {}
    _RockScale("Rock Scale", Float) = 0.1
    _RockColorBright("Rock Color Bright", Color) = (1,1,1,1)
    _RockColorDark("Rock Color Dark", Color) = (1,1,1,1)
	[NoScaleOffset] _Sediment("Sediment Texture", 2D) = "white" {}
    _SedimentScale("Sediment Scale", Float) = 0.1
    _SedimentIntensity("Sediment Intensity", Range(0, 1)) = 0.1
    _SedimentDistortion("Sediment Distortion", Float) = 1

    [Header(Dirt Settings)]
	[NoScaleOffset] _Dirt("Dirt Layer Texture", 2D) = "white" {}
    _DirtScale("Dirt Scale", Float) = 0.1
    _DirtColorBright("Dirt Color Bright", Color) = (1,1,1,1)
    _DirtColorDark("Dirt Color Dark", Color) = (1,1,1,1)
    _DirtSlopeMin("_Dirt Slope Min", Range(0, 1)) = 0
    _DirtSlopeMax("_Dirt Slope Max", Range(0, 1)) = 1

    [Header(Grass Settings)]
	[NoScaleOffset] _Grass("Grass Layer Texture", 2D) = "white" {}
    _GrassScale("Grass Scale", Float) = 0.1
    _GrassColorBright("Grass Color Bright", Color) = (1,1,1,1)
    _GrassColorDark("Grass Color Dark", Color) = (1,1,1,1)
    _GrassSlopeMin("Grass Slope Min", Range(0, 1)) = 0
    _GrassSlopeMax("Grass Slope Max", Range(0, 1)) = 1
    _GrassHeightMin("Grass Height Min", Float) = 300
    _GrassHeightMax("Grass Height Max", Float) = 400
    _GrassHeightMod("Grass Height Mod", Float) = 5
    ...
    _SnowSlopeMin("Snow Slope Min", Range(0, 1)) = 0
    _SnowSlopeMax("Snow Slope Max", Range(0, 1)) = 1
    _SnowHeightMin("Snow Height Min", Float) = 500
    _SnowHeightMax("Snow Height Max", Float) = 600
    _SnowHeightMod("Snow Height Mod", Float) = 5
	...
	[Toggle] _IncludeSnow("Include Snow", Float) = 1
...
#define _UNLIT 1
    ...
    float    _RockScale;
    half3   _RockColorBright;
    half3   _RockColorDark;
    float    _SedimentScale;
    half    _SedimentIntensity;
    half    _SedimentDistortion;
    float    _DirtScale;
    half3   _DirtColorBright;
    half3   _DirtColorDark;
    half    _DirtSlopeMin;
    half    _DirtSlopeMax;
    float    _GrassScale;
    half3   _GrassColorBright;
    half3   _GrassColorDark;
    half    _GrassSlopeMin;
    half    _GrassSlopeMax;
    half    _GrassHeightMin;
    half    _GrassHeightMax;
    half    _GrassHeightMod;
    half3   _FlowColor;
    half    _FlowIntensity;
    half3   _SnowColor;
    half    _SnowSlopeMin;
...
#define unity_WorldToObject GetWorldToObjectMatrix()
	...
	void Ext_SurfaceFunction0 (inout Surface o, ShaderData IN)
	{
        ...
        half4 splatmap = tex2D(_SplatMap, IN.texcoord0.xy);
        // Layer Textures
        half4 rock  = tex2D(_Rock,  uv / _RockScale);
        half4 dirt  = tex2D(_Dirt,  uv / _DirtScale);
        half4 grass = tex2D(_Grass, uv / _GrassScale);
        half sediment = lerp(1, tex2D(_Sediment, float2(0, wPos.y + splatmap.x * _SedimentDistortion) / _SedimentScale).a, _SedimentIntensity);

		half4 detail = tex2D(_Dirt, uv / _DetailScale);
        ...
        // Layer Matrices
        half2x3 mrock  = half2x3(rock.xyz,  lerp(_RockColorDark,  _RockColorBright,  rock.a) * sediment );
        half2x3 mdirt  = half2x3(dirt.xyz,  lerp(_DirtColorDark,  _DirtColorBright,  dirt.a));
        half2x3 mgrass = half2x3(grass.xyz, lerp(_GrassColorDark, _GrassColorBright, grass.a));

        // Parameters
        half dirtSlope   = saturate( (normal.y - _DirtSlopeMin  ) / (_DirtSlopeMax   - _DirtSlopeMin  ) );
        half grassSlope  = saturate( (normal.y - _GrassSlopeMin ) / (_GrassSlopeMax  - _GrassSlopeMin ) );
        half grassHeight = saturate( ((wPos.y - splatmap.r*_GrassHeightMod) - _GrassHeightMax) / (_GrassHeightMin - _GrassHeightMax) );

        // Lerping
        half lerpValDirt  = saturate(splatmap.r * dirtSlope );
        half lerpValGrass = saturate(splatmap.g * grassSlope) * grassHeight;
        half2x3 m = lerp(lerp(mrock, mdirt, lerpValDirt), mgrass, lerpValGrass);

		_FlowIntensity *= saturate(length(float3(unity_ObjectToWorld[0].y, unity_ObjectToWorld[1].y, unity_ObjectToWorld[2].y)));
        // Albedo
        half3 albedo = lerp(m[1], lerp(m[1], _FlowColor, splatmap.a), _FlowIntensity) * detail.a;

        // Normal
        float3 n = float3( (m[0].xy * 2 - 1) + detail.xy, 1);
        n.z = sqrt(1 - saturate(dot(n.xy, n.xy)));
		n = n.xzy * float3(-1, 1, -1); 
		//n = UnityObjectToWorldNormal(n.xyz); 
		normal = normalize(float3(normal.x + n.x, normal.y * n.y, normal.z + n.z));

        // Snow
        half snowSlope = saturate((normal.y - _SnowSlopeMin) / (_SnowSlopeMax - _SnowSlopeMin));
        half snowHeight = saturate(((wPos.y + splatmap.x*_SnowHeightMod) - _SnowHeightMin) / (_SnowHeightMax - _SnowHeightMin));
        albedo = lerp(albedo, _SnowColor, snowSlope * snowHeight * _IncludeSnow);

        // Occlusion
        half occlusion = min( lerp(1, min(splatmap.b, m[0].z), _OcclusionIntensity), detail.b);
