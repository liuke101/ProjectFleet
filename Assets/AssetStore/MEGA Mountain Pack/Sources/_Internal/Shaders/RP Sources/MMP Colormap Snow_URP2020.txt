...
#define unity_WorldToObject GetWorldToObjectMatrix()
	...
	void Ext_SurfaceFunction0 (inout Surface o, ShaderData IN)
	{
        ...
        // Main Textures
		float4 temp = tex2D(_BaseMap, IN.texcoord0.xy);
		float3 normal   = (temp.xzy * 2 - 1) * float3(-1, 1, -1);
		half4 colormap = tex2D(_ColorMap, IN.texcoord0.xy);

        // Albedo
        half3 albedo = colormap.rgb;
        ...
        half snowHeight = saturate((wPos.y + temp.a * _SnowHeightMod - _SnowHeightMin) / (_SnowHeightMax - _SnowHeightMin));
        albedo = lerp(albedo, _SnowColor, snowSlope * snowHeight);

        // Occlusion
        half occlusion = lerp(1, colormap.a, _OcclusionIntensity);
