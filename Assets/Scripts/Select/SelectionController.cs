    ...
    public void MoveTo()
    {
        ...
        if (Input.GetMouseButtonDown(0))
        {
                    ...
                    1 << LayerMask.NameToLayer("Water")))
            {
                ...
                {
                    ShipAutopilotController shipAutopilotController = selectedShips[i].GetComponent<ShipAutopilotController>();
                    if (shipAutopilotController)
                    {
                        //船朝向阵列目标移动
                        Debug.DrawLine(TargetPositions[i], TargetPositions[i] + Vector3.up * 100, Color.red, 1.0f);
                        shipAutopilotController.StartMove(TargetPositions[i]);
                    }
                }
            }
        }
